...
watch(newssResponse,(vr:any)=>{
  if (vr.data?.data){
    newss.value.push(...vr?.data?.data)
  }
})

function likeToggle(v:any) {
  const item = newss.value.find((item:any) => item._id === v._id);
  $fetch(endpoints.like.toggle,{
    method:"post",
    body:{
      target:"news",target_id:v._id
    },
    headers:pitLib.util.headers(),
  }).then((d:any)=>{
    if(d.status){
      ...
      <div class="middle-sidebar-left">
        ...
        <div class="row feed-body">
          <div class="col-xl-8 col-xxl-9 col-lg-8">
            <div v-for="v in newss" class="card data-card w-100 shadow-xss rounded-xxl border-0 p-4 mb-3">
              <NuxtLink :to="'/news/'+v?.slug">
...
<!--                </a>-->
                ...
                <a role="button" @click="loadComments(v)" class="d-flex align-items-center fw-600 text-grey-900 text-dark lh-26 font-xssss"><i class="feather-message-circle text-dark text-grey-900 btn-round-sm font-lg"></i><span class="d-none-xss">{{v?.comments?.[0]?.total??0}} Comment</span></a>
            ...
            <div v-if="newssResponse?.data?.pages>page" class="row mt-2">
              <div class="col text-center">
                <button @click="page++;newssRefetch()" v-if="!loadingnewssResponse" class="btn btn-primary btn-lg text-white rounded shadow-lg">
                  Load More
                  <i class="fa-solid fa-angle-down ms-2"></i>
                </button>
                <client-only>
                  <Vue3Lottie
                      v-if="loadingnewssResponse"
