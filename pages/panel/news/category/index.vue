...
const page=ref(1)
const swal=useSwal()
const {data:categsResponse,loading:loadingCategsResponse,refresh:refetch} =useFetch<PaginatedApiResponse>(endpoints.news.category.index,{
  method:"post",
  body:{
    // status:"pending"
  },
...
function confDel(v:any){
  swal.fire({
    ...
    if (result.isConfirmed) {
      $fetch(endpoints.news.category.delete,{
        method:"post",
        body:{
          _id:v?._id,
          status:"rejected"
...
<template>
  <PanelHeader title="News Categories" description="List of News Categories" icon="outdent"></PanelHeader>
