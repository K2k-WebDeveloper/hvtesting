...
const {data:blogsResponse,loading:loadingBlogsResponse,refresh:refetch} =useFetch<PaginatedApiResponse>(endpoints.news.index,{
  method:"post",
  body:{
                ...
                <div class="col-12 col-sm-3 col-md-3 col-lg-3 col-xl-4">
                  <div v-if="blogsResponse?.data?.data?.[0]?.media?.length" class="card-body p-0 mb-3 rounded-3 overflow-hidden">
                    <div v-for="media in blogsResponse?.data?.data?.[0]?.media" class="">
                      <img  @error="event.target.src=env.NO_IMG" v-if="media?.ref_code=='news_image'" :src="media?.is_local?env.BASEPOINT+ media?.path:media?.path" class="img-fluid mb-2">
                    </div>
                  </div>
                </div>
              </div>
