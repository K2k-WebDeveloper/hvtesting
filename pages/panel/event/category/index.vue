...
const page=ref(1)
const swal=useSwal()
const {data:categsResponse,loading:loadingCategsResponse,refresh:refetch} =useFetch<PaginatedApiResponse>(endpoints.event.category.index,{
  method:"post",
  body:{
    // status:"pending"
  },
...
function confDel(v:any){
  swal.fire({
    ...
    if (result.isConfirmed) {
      $fetch(endpoints.event.category.delete,{
        method:"post",
        body:{
          _id:v?._id,
          status:"rejected"
...
<template>
  <PanelHeader title="Event Categories" description="List of Event Categories" icon="outdent"></PanelHeader>
