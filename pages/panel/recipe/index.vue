...
watch(recipesResponse,v=>{
  if (recipesResponse.value.status){
    selTargets.value=recipesResponse.value.data.data
  }
})
...
<template>
  <PanelHeader title="Recipes" description="Approve or Reject uploaded recipes by users." icon="recipe"></PanelHeader>
        ...
        <div class="card _lift h-100" >
          ...
          <div class="card-body d-flex justify-content-center flex-column">
            ...
            <q-markup-table>
              ...
              <tbody v-if="recipesResponse?.status">
              <tr v-for="v in recipesResponse?.data?.data" :key="n">
                ...
                <td class="text-left">
                  <q-btn size="sm" :to="'/panel/recipe/'+v?._id" class="m-2" round color="info" icon="fa fa-eye"></q-btn>
              ...
              <ui-table-skeleton v-if="loadingRecipesResponse" :rows="3" :columns="5"></ui-table-skeleton>

              </tbody>
            </q-markup-table>
            <q-pagination
                class="mt-3"
                v-if="recipesResponse?.status"
                v-model="page"
                :max="recipesResponse?.data?.pages"
            />
          </div>
        </div>
      </div>
